...
// Create a new account. Optionally set a password so they can sign in at once;
// otherwise email them an invite to set their own.
export async function createAccount(input: {
  ...
  const wantsPassword = !!input.password && input.password.length > 0;
  if (wantsPassword && (input.password as string).length < 8)
  ...
  if (invErr || !uid) {
    ...
    // Only send a recovery email if we're NOT setting a password directly below.
    if (!wantsPassword) {
  ...
  // If a password was set at creation, apply it directly so they can sign in
  // right away, no email needed.
  if (wantsPassword && uid) {
